...
<template>
  <div class="ui-kit-sidebar">
    ...
    <div class="md:hidden bg-white dark:bg-gray-800 p-4 sticky top-0 z-10">
      <div @click="toggleMobileMenu" class="flex items-center justify-between cursor-pointer">
        <h2 class="body-1 text-color font-medium">{{ navigationItems.find(item => item.id === activeTab)?.name }}</h2>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-color transform transition-transform" 
          :class="{ 'rotate-180': mobileMenuOpen }" viewBox="0 0 20 20" fill="currentColor">
          <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
        </svg>
      </div>
      <div v-if="mobileMenuOpen" class="mt-2 py-2 rounded-md shadow-lg bg-white dark:bg-gray-700 absolute w-full left-0 z-20">
